...
static std::vector<libnest2d::Item>& qidiParts() {
    using namespace libnest2d;
...
TEST_CASE("QIDIPartsShouldFitIntoTwoBins", "[Nesting]") {

    // Get the input items and define the bin.
    std::vector<Item> input = qidiParts();
    ...
    // For qidi parts, 2 bins should be enough...
    REQUIRE(bins > 0u);
    REQUIRE(bins <= 2u);
