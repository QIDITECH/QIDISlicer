...
void Tab::update_mode()
{
    ...
    update_visibility();
    update_sla_qidi_specific_visibility();
...
//Y16//Y26
        bool chamber_temp = printer_config->opt_bool("chamber_temperature_control") && m_config->opt_bool("seal_print");
        toggle_option("chamber_temperature", chamber_temp);
        toggle_option("chamber_minimal_temperature", chamber_temp);
        if (!chamber_temp) {
            DynamicPrintConfig new_conf = *m_config;
            new_conf.set_key_value("chamber_temperature", new ConfigOptionInts{0});
...
void TabPrinter::update_sla_qidi_specific_visibility()
{
    if (m_active_page && m_active_page->title() == "General") {
        ...
        if (og_it != m_active_page->m_optgroups.end()) {            
            og_it->get()->Show(m_mode == comExpert && !is_qidi_printer());
            Layout();
        }
    }
}
