    ...
    m_extra_style(style == wxSL_VERTICAL ? wxSL_AUTOTICKS | wxSL_VALUE_LABEL : 0)
{
    ...
    LIGHT_BLUE_PEN    = wxPen(wxColour(68, 121, 251));
    ...
    m_segm_pens = { &LIGHT_BLUE_PEN, &LIGHT_BLUE_PEN, &LIGHT_BLUE_PEN };
...
void Control::draw_tick_text(wxDC& dc, const wxPoint& pos, int tick, LabelType label_type/* = ltHeight*/, bool right_side/*=true*/) const
{
    ...
    //B18
    wxColour old_clr = dc.GetTextForeground();
    const wxPen& pen = is_wipe_tower_layer(tick) && (tick == m_lower_value || tick == m_higher_value) ? LIGHT_BLUE_PEN : /*wxPen(old_clr)*/GREY_PEN;
    dc.SetPen(pen);
