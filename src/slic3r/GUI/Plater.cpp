...
#include "slic3r/Utils/QIDIConnect.hpp"
...
//                wxGetApp().sidebar().update_ui_from_settings();
    ...
    if (
        qidislicer_generator_version
        && *qidislicer_generator_version < Semver("2.9.0-alpha1")
    ) {
...
//y20
ThumbnailData Plater::get_thumbnailldate_from_bed(int num)
{
    return p->thumbnails[num];
}

int Plater::get_beds_num() {
    return s_multiple_beds.get_number_of_beds();
};

int Plater::get_active_bed() {
    return s_multiple_beds.get_active_bed();
};
...
void Plater::connect_gcode_all() {
    ...
    const auto print_host_ptr{dynamic_cast<QIDIConnectNew*>(upload_job_template.printhost.get())};
        ...
        with_mocked_fff_background_process(
            *print,
            ...
            [&](){
                ...
                upload_job.printhost = std::make_unique<QIDIConnectNew>(connect);
