        ...
        if (ImGui::CollapsingHeader(name.c_str(), ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            if (method == 0 || method == 1) {
                ...
                if (svd.rotation) {
                    ...
                    imgui.text_colored(ImGuiWrapper::COL_BLUE_LIGHT, svd.rotation_90_degrees ? "Rotation 90 degs" : "Rotation");
                    spacing = 10.0f;
                }
                if (svd.scale) {
                    ImGui::SameLine(0.0f, spacing);
                    imgui.text_colored(ImGuiWrapper::COL_BLUE_LIGHT, svd.anisotropic_scale ? "Anisotropic scale" : "Isotropic scale");
                    spacing = 10.0f;
                }
                if (svd.mirror) {
                    ImGui::SameLine(0.0f, spacing);
                    imgui.text_colored(ImGuiWrapper::COL_BLUE_LIGHT, "Mirror");
                    spacing = 10.0f;
                }
                if (svd.skew) {
                    ImGui::SameLine(0.0f, spacing);
                    imgui.text_colored(ImGuiWrapper::COL_BLUE_LIGHT, "Skew");
                }
            }
        }
    };
