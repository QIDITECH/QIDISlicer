...
void PageMaterials::set_compatible_printers_html_window(const std::vector<std::string>& printer_names, bool all_printers)
{
    ...
    if (materials->technology == T_FFF && template_shown) {
        ...
        for (const Preset *printer : materials->printers) {
            if (printer->vendor && printer->vendor->id == "QIDIProMedical") {
                has_medical = true;
            }
        }
        // TRN QIDISlicer-Medical ConfigWizard: Materials"
        wxString zero_line = _L("The list of validated workflows for Medical One can be found in this <a href=\"qidi.io/m1-validation\">article</a>. Profiles for other materials are not verified by the material manufacturer and therefore may not correspond to the current version of the material.");
        if (!has_medical) {
            zero_line.Clear();
        }
        // TRN ConfigWizard: Materials : "%1%" = "Filaments"/"SLA materials"
...
void ConfigWizard::priv::create_vendor_printers_page(const std::string& repo_id, const VendorProfile* vendor, bool install/* = false*/, bool from_single_vendor_repo /*= false*/)
{
    ...
    const bool is_qidi_vendor = vendor->name.find("QIDI") != std::string::npos;
    const unsigned indent = from_single_vendor_repo ? 0 : 1;

    if (is_fff_technology) 
    {
        pageFFF = new PagePrinters(q, vendor->name + " " +_L("FFF Technology Printers"), vendor->name + (is_qidi_vendor ? "" : " FFF"), *vendor, indent, T_FFF);
    ...
    if (is_sla_technology) 
    {
        pageSLA = new PagePrinters(q, vendor->name + " " + _L("SLA Technology Printers"), vendor->name + (is_qidi_vendor ? "" : " MLSA"), *vendor, indent, T_SLA);
...
void ConfigWizard::priv::load_pages_from_archive()
{
    ...
    for (const auto* archive : archs) {
        ...
        if (!vendors.empty())
        {
            ...
            const bool is_non_qidi = data.id.find("non-qidi") == 0;
            if (is_non_qidi || vendors.size() > 1)
            {
                // it's multiple vendor or non-qidi repository
            ...
            {
                // it's single qidi vendor repository
