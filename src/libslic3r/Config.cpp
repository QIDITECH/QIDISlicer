...
std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const
{
	...
	if (this->cli != ConfigOptionDef::nocli) {
        ...
        // Neither '=' nor '!' is used in any of the cli parameters currently defined by QIDISlicer.
...
bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value, ConfigSubstitutionContext& substitutions_ctxt, bool append)
{
    ...
    if (optdef->type == coBools && substitutions_ctxt.rule != ForwardCompatibilitySubstitutionRule::Disable) {
    	//FIXME Special handling of vectors of bools, quick and not so dirty solution before QIDISlicer 2.3.2 release.
...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by QIDISlicer ..." comment line will be removed.
    size_t j = 0;
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &filename, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char qidislicer_gcode_header[] = "; generated by QIDISlicer ";
        std::string header;
        ...
        while (std::getline(ifs, header)) {
            if (strncmp(slic3r_gcode_header, header.c_str(), strlen(slic3r_gcode_header)) == 0) {
                header_found = true;
                break;
            } else if (strncmp(qidislicer_gcode_header, header.c_str(), strlen(qidislicer_gcode_header)) == 0) {
                // Parse QIDISlicer version.
                size_t i = strlen(qidislicer_gcode_header);
        ...
        if (! header_found)
            throw Slic3r::RuntimeError("Not a QIDISlicer / Slic3r PE generated g-code.");
    }
    ...
    if (has_delimiters)
    {
        // QIDISlicer starting with 2.4.0-alpha0 delimits the config section stored into G-code with 
        ...
        if (! begin_found) 
            throw Slic3r::RuntimeError(format("Configuration block opening tag \"; qidislicer_config = begin\" not found when reading %1%", filename));
    }
    else
    {
        // Slic3r or QIDISlicer older than 2.4.0-alpha0 do not emit any delimiter.
