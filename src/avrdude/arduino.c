...
static int qidi_init_external_flash(PROGRAMMER * pgm)
{
  ...
  if (res < 0) {
    avrdude_message(MSG_INFO, "%s: qidi_init_external_flash(): MK3 printer did not boot up on time or serial communication failed\n", progname);
    return -1;
  } else if (strncmp(buffer, entry_magic_send, recv_size) != 0) {
    avrdude_message(MSG_INFO, "%s: qidi_init_external_flash(): MK3 printer emitted incorrect start code: `%*s`\n", progname, recv_size, buffer);
    return -1;
  }
  ...
  if (serial_send(&pgm->fd, entry_magic_receive, sizeof(entry_magic_receive) - 1) < 0) {
    avrdude_message(MSG_INFO, "%s: qidi_init_external_flash(): Failed to send command to the printer\n",progname);
    return -1;
  }
  ...
  if (res < 0) {
    ...
    avrdude_message(MSG_INFO, "%s: qidi_init_external_flash(): MK3 printer emitted incorrect cfm code: `%*s`\n", progname, recv_size, buffer);
    return -1;
  }
...
static int arduino_open(PROGRAMMER * pgm, char * port)
{
  ...
  // Initialization sequence for programming the external FLASH on the QIDI MK3
  if (qidi_init_external_flash(pgm) < 0) {
