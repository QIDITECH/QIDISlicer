...
// static int page = 0;
  ...
  // QIDI3D workaround for a bug in the USB communication controller. The semicolon character is used as an initial character
  ...
  bool qidi3d_semicolon_workaround_round2 = false;
  // Buffer containing the other nibbles of semicolons to be flashed in the 2nd round.
  unsigned char qidi3d_semicolon_workaround_round2_data[256];
  ...
  for (; addr < maxaddr; addr += qidi3d_semicolon_workaround_round2 ? 0 : page_size) {
    ...
    if((last_addr==UINT_MAX)||(last_addr+block_size != addr)||qidi3d_semicolon_workaround_round2){
      if (stk500v2_loadaddr(pgm, use_ext_addr | (addr >> addrshift)) < 0)
        return -1;
    }
    last_addr=addr;

    if (qidi3d_semicolon_workaround_round2) {
        // printf("Round 2: address %d\r\n", addr);
        memcpy(buf+10, qidi3d_semicolon_workaround_round2_data, block_size);
        qidi3d_semicolon_workaround_round2 = false;
    } else {
        for (size_t i = 0; i < block_size; ++ i) {
            unsigned char b = m->buf[addr+i];
            if (b == ';') {
              // printf("semicolon at %d %d\r\n", addr, i);
              qidi3d_semicolon_workaround_round2_data[i] = b | 0x0f0;
              b |= 0x0f;
              qidi3d_semicolon_workaround_round2 = true;
            } else 
              qidi3d_semicolon_workaround_round2_data[i] = 0x0ff;
            buf[i+10] = b;
        }
    }
