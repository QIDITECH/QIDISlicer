...
@ECHO                  console - run and wait on qidi-slicer-console.exe
...
@ECHO                  window - run qidi-slicer.exe (no wait)
@ECHO                Default: none
...
SET START_TIME=%TIME%
SET PS_START_DIR=%CD%
SET PS_SOLUTION_NAME=QIDISlicer
...
IF "%PS_RUN%" EQU "console" (
    icacls qidi-slicer-console.exe >nul || GOTO :END
    start /wait /b qidi-slicer-console.exe
) ELSE IF "%PS_RUN%" EQU "window" (
    icacls qidi-slicer.exe >nul || GOTO :END
    start qidi-slicer.exe
