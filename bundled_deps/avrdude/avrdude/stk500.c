...
static int stk500_loadaddr(PROGRAMMER * pgm, AVRMEM * mem, unsigned int addr)
{
 ...
 retry:
  ...
  // Workaround for the infamous ';' bug in the QIDI3D usb to serial converter.
                              ...
                              unsigned int addr, unsigned int n_bytes)
{
  ...
  unsigned int n;
  unsigned int i, j;
  unsigned int qidi3d_semicolon_workaround_round = 0;
  ...
  retry:
    ...
    for (qidi3d_semicolon_workaround_round = 0; qidi3d_semicolon_workaround_round < (has_semicolon ? 2u : 1u); qidi3d_semicolon_workaround_round++) {
      ...
      if (has_semicolon) {
        for (j = 0; j < (unsigned)block_size; ++i, ++ j) {
          buf[i] = m->buf[addr + j];
          if (buf[i] == ';')
            buf[i] |= (qidi3d_semicolon_workaround_round ? 0xf0 : 0x0f);
        }
      } else {
